...
function MainGroup({ route, navigate }) {
  ...
  const HandleClick = async () => {
    ...
    const result = await getAllSubGroup(tFValue, fromDate, toDate);
    if (result.status === 200) {
      //console.log(result.data.ResultObject);
      setSubGroupData(result.data.ResultObject.subGroupList);
      setLoading(true);
    }
  };
  const HandleGetMainGroup = async () => {
    const result = await getAllMainGroupNoCondition();
    if (result.status === 200) {
      //console.log(result.data.ResultObject);
  ...
  const handleDeleteSubGroup = async (item) => {
    ...
    setLoading(false);
    const result = await deleteSubGroup(item.SUBGROUPID);
    if (result.status === 200) {
      setLoading(true);
      HandleClick();
    }
  ...
  const handleEditClick = (item) => {
    ...
    if (item.ISCHECKSTOCKQUANTITY === 1) setCheckedStockEdit(true);
    else {
      setCheckedStockEdit(false);
    }
    if (item.ISCANRETURNOUTPUT === 1) setIsCanReturnOutputEdit(true);
    else {
      setIsCanReturnOutputEdit(false);
    }
    if (item.ISAUTOCREATEIMEI === 1) setIsAutoCreateIMEIEdit(true);
    else {
      setIsAutoCreateIMEIEdit(false);
    }
  ...
  const handleAgrreEdit = async () => {
    ...
    //   checkedIMEIEdit,
    //   checkedStockEdit,
    //   isCanReturnOutputEdit,
    //   isAutoCreateIMEIEdit,
    //   tFDesEditValue,
    //   checkedActiveEdit,
    //   1,
    //   toDate
          ...
          <Box sx={style}>
            ...
            <div style={{ marginLeft: 37 }}>
              <div className="row">
                ...
                <Checkbox
                  className="col-1"
                  checked={checkedStockEdit}
                  onChange={handleChangeStockEdit}
                  inputProps={{ "aria-label": "controlled" }}
                />
              </div>
              <div className="row">
                ...
                <Checkbox
                  className="col-1"
                  checked={isCanReturnOutputEdit}
                  onChange={handleChangeReturnOutputEdit}
                  inputProps={{ "aria-label": "controlled" }}
                />
              </div>
              <div className="row">
                ...
                <Checkbox
                  className="col-1"
                  checked={isAutoCreateIMEIEdit}
                  onChange={handleChangeAutoCreateIMEIEdit}
                  inputProps={{ "aria-label": "controlled" }}
                />
              </div>
            </div>
